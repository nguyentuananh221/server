...
const ChallengerType = require('../models/chanllengerType.model')
const Challenger = require('../models/challenger.model')
...
const challengerController = {
    ...
    addChallenger : async (req, res) => {
        const {classID,typeID, name, note} = req.body

        if (!classID || !typeID ||!name) {
            return res.status(400).json(createError(false,'Thieu thong tin bat buoc'))
        }
        try {
    ...
    deleteChallenger : async (req, res) => {
        const {id } = req.params
        const findChallenger = await Challenger.findById(id)
        // console.log(findChallenger)
        try {
            await console.log(findChallenger)
            if (!findChallenger) {
                return res.status(400).json(createError(false,'Khong tim thay thu thach'))
            }
            if (findChallenger.sentStatus === true) {
                return res.status(200).json(createError(false,'Khong the xoa thu thach vi da gui di'))
            }
    ...
    getChallengerByID : async (req,res) => {
        const {id} = req.params
        try {
            const detailchallenger = await Challenger.findById(id)
